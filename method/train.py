...
from method.data_provider import Dataset4Training,VisDataSet,\
    TxtDataSet,collate_train,read_video_ids
...
def train_epoch(model, train_loader, optimizer, opt, epoch_i, training=True):
    ...
    loss_meters = OrderedDict(clip_nce_loss=AverageMeter(), clip_trip_loss=AverageMeter(),
                            atten_nce_loss=AverageMeter(), atten_trip_loss=AverageMeter(),
                              loss_overall=AverageMeter(), inter_video_trip_loss=AverageMeter())
    ...
    for batch_idx, batch in tqdm(enumerate(train_loader), desc="Training Iteration", total=num_training_examples):
        ...
        timer_start = time.time()
        loss, loss_dict = model(batch['frame_video_features'],batch['videos_mask'],
...
def start_training(opt):
    ...
    train_dataset = Dataset4Training(caption_files['train'], video_feat_path, text_feat_path, opt)

    val_text_dataset = TxtDataSet(caption_files['val'], text_feat_path, opt)

    val_video_ids_list = read_video_ids(caption_files['val'])
    val_video_dataset = VisDataSet(video_feat_path, opt, video_ids=val_video_ids_list)
    ...
    model_config = EDict(
        ...
        global_ksize=opt.global_ksize,
        global_stride=opt.global_stride,
